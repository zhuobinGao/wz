...
var myUrl = "../../../WZCangKu/servlet/DataRequestServlet?param=preList";
function initTable(){
...
function initDel(){
	...
	$('#delEdit').click(function(){
		...
		console.info(table.rows( { selected: true } ).data()[0]);
	 ...
	 $('#d_save').click(function(){
		 ...
		 for(var i=0;i<count;i++){
			 data += (table.rows({selected:true}).data()[i][1]+",") ;
		 }
		 
		 
		 $.post("../../../WZCangKu/servlet/DataRequestServlet?param=closePre&data="+data,function(data1){
...
function initOpen(){
	$('#s_open').click(function(){
		var count = table.rows( { selected: true } ).count();
		 ...
		 $.post("../../../WZCangKu/servlet/DataRequestServlet?param=openPre&data="+data,function(data1){
			 alert(data1);
			 table.ajax.url( myUrl).load();
		 })
